...
type Video struct {
	...
	CommentCount  int64                     `json:"comment_count" db:"comment_count"`   // 视频的评论总数
	FavoriteCount int64                     `json:"favorite_count" db:"favorite_count"` // 视频的点赞总数
	IsFavorite    bool                      `json:"is_favorite" db:"is_favorite"`       // true-已点赞，false-未点赞
	CreatedAt     int64                     `json:"-" db:"create_at"`                   // 视频创建时间
	ModifiedAt    int64                     `json:"-" db:"modified_at"`
	User          `json:"author" db:"user"` // 视频作者信息
}
